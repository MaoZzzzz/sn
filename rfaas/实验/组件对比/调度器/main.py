...
def success_ratio():
    ...
    plt.plot(ind, rfaas, marker='*', label='AGLBSA', linewidth=2)

    plt.xticks(ind, labels)
    plt.tick_params(axis='both', which='major', labelsize=13, length=0)
    # plt.margins(x=0.05)

    plt.legend(fontsize=13, loc='lower left', fontproperties='Times New Roman')
    plt.ylabel('调度成功率', labelpad=5, fontsize=13)
...
def success_num():
    ...
    plt.ylabel('调度成功数量', labelpad=5, fontsize=13)
    # plt.xlabel('负载类型', labelpad=5)
    plt.grid(True, linestyle='--')
    plt.subplots_adjust(left=0.15)

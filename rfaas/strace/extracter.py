...
def get_timeline(func_name, start, end, solo_lat):
    ...
    with open(f"{func_name}/data/{func_name}.csv", "r") as f:
    ...
    for line in raw_lines:
        ...
        if not format_line == "":
            print(format_line)
            print(format_line[1] + format_line[3])
            print(latency)
    ...
    with open(f"{func_name}/data/{func_name}_format.csv", "w") as f:
        f.writelines(str(res))

if __name__ == '__main__':
    get_timeline("matmul", 1703576264.918656, 1703576266.357777, 1703576267.9)
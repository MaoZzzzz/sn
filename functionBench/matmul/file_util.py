...
FILE_PATH_PREFIX = "D:\\Workdir\\pycharm\\sn\\functionBench\\matmul\\data"
...
def get_file_path(n):
    ...
    for str in riscv_files:
        tmp = str.split("-")
        if n == tmp[1] and "b.txt" == tmp[3]:
            riscv_file_name_b = FILE_PATH_PREFIX + "\\riscv" + "\\" + str

        if n == tmp[1] and "u.txt" == tmp[3]:
...
def read_file(n, type, arch):
    riscv_file_name_b, riscv_file_name_u, x86_file_name_b, x86_file_name_u = get_file_path(n)
    ...
    if type == "cpu":
        return user_result, idle_result
    else:
...
if __name__ == '__main__':
    user_result, idle_result = read_file("200", "cpu", "riscv")
    print(user_result)
    print(idle_result)

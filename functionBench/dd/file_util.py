...
FILE_PATH_PREFIX = "D:\\Workdir\\pycharm\\sn\\functionBench\\dd\\data"


def get_file_path(bs, count):
    riscv_file_name_b = ""
    riscv_file_name_u = ""
    x86_file_name_b = ""
    x86_file_name_u = ""
    ...
    for str in riscv_files:
        tmp = str.split("-")
        if bs == tmp[1] and count == tmp[2] and "b.txt" == tmp[4]:
            riscv_file_name_b = FILE_PATH_PREFIX + "\\riscv" + "\\" + str

        if bs == tmp[1] and count == tmp[2] and "u.txt" == tmp[4]:
...
def read_file(bs, count, type, arch):
    riscv_file_name_b, riscv_file_name_u, x86_file_name_b, x86_file_name_u = get_file_path(bs, count)
...
if __name__ == '__main__':
    user_result, idle_result, iowait_result = read_file("4096", "10000", "cpu", "riscv")
    print(user_result)
    print(idle_result)
